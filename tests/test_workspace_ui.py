...
def test_workspace_uses_only_implemented_navigation_and_states() -> None:
    page = (STATIC_DIR / "index.html").read_text(encoding="utf-8")
    script = (STATIC_DIR / "app.js").read_text(encoding="utf-8")

    assert "Researching" not in page
    assert "Researching" not in script
    assert "Needs Input" in page
    assert "Calculating" in script
    assert "Review" in script
    assert "Ready" in script
    assert "Web Search" not in page
